	...
	<view class="" >
		<view class="proRowList whiteBj">
			<view class="item pdtb15 mglr4 flexRowBetween" v-for="item in mainData">
				...
				<view class="rr">
					...
					<view class="Bmny">
						<view class="price fs14">{{item.product&&item.product.title?item.product.price:''}}/天</view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="myRowBetween fs13 whiteBj">
			...
			<view class="item flexRowBetween" style="align-items: flex-start;">
				...
				<view style="width: 80%;">
					<view class="flexEnd seltDay center fs13">
						<view class="btn" :class="currChoose==1?'on':''" @click="choose('1')">1天</view>
						<view class="btn" :class="currChoose==2?'on':''" @click="choose('2')">2天</view>
						<view class="btn" :class="currChoose==3?'on':''" @click="choose('3')">自定义</view>
					</view>
					<view class="flexEnd">
						<view class="borderB1 pdb5 pdt5" style="width: 320rpx;" v-if="currChoose==3">
			...
			<view class="item flexRowBetween" @click="Router.navigateTo({route:{path:'/pages/user_address/user_address'}})">
				<view class="ll" style="width: 20%;">服务地址</view>
				<view class="rr" style="width: 80%;" v-if="addressData.city">
					<view class="cont">{{addressData.city+addressData.detail}}</view>
					<image class="arrowR" src="../../static/images/arrow-icon.png" alt=""/>
				</view>
				<view class="rr" style="width: 80%;" v-else>
	...
	export default {
		...
		methods: {
			...
			countTotalPrice() {
				...
				self.totalPrice = parseFloat(self.productPrice) + self.distancePrice;
